...
class Renderer:
    ...
    def __call__(
        ...
        top_view=False,
        rot_angle=90,
        mesh_base_color=(1.0, 1.0, 0.9),   # can be list of (r,g,b)
        scene_bg_color=(0, 0, 0),
        tri_color_lights=False,
        return_rgba=False,
        camera_center=None,
        ...
        Args:
            vertices (np.array): Concatenated mesh vertices of all persons (N*V, 3).
            cam_t (np.array): Camera translation (3,).
            image (np.array): Input RGB image (H, W, 3) in [0,255].
        """

        # Load full-frame image if required
        if full_frame:
            image = cv2.imread(imgname).astype(np.float32)
        image = image / 255.0
        h, w = image.shape[:2]
        ...
        # ------------------------------------------------------
        # Build trimesh with per-person vertex colors
        # ------------------------------------------------------
        verts = vertices.copy()
        faces = self.faces.copy()

        # Each person has a fixed number of SMPL vertices
        V = 18439         # fixed vertex count per person
        N = verts.shape[0] // V   # number of persons

        # Create vertex color array
        vertex_colors = np.zeros((verts.shape[0], 4), dtype=np.uint8)

        # Assign color per person by fixed index ranges
        for pid in range(N):
            start = pid * V
            end = start + V
        ...
        # Return RGBA directly if requested
        if return_rgba:
            return color
