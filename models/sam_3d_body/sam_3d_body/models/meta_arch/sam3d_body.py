...
class SAM3DBody(BaseModel):
    ...
    def run_inference_batch(
        ...
        for bi in batch_lhand_list:
            for k, v in bi.items():
                if k == 'bbox_format':
                    continue
                if k == 'img_ori':
                    v = v[0]
                if k not in batch_lhand_dict:
                    batch_lhand_dict[k] = [v]
                else:
                    batch_lhand_dict[k].append(v)

        for k, v in batch_lhand_dict.items():
            if k == 'img_ori':
                continue
            batch_lhand_dict[k] = torch.concat(v, dim=0)
        batch_lhand_dict['bbox_format'] = batch_lhand_list[0]['bbox_format']
        batch_lhand = recursive_to(batch_lhand_dict, "cuda")
        ...
        for bi in batch_rhand_list:
            for k, v in bi.items():
                if k == 'bbox_format':
                    continue
                if k == 'img_ori':
                    v = v[0]
                if k not in batch_rhand_dict:
                    batch_rhand_dict[k] = [v]
                else:
                    batch_rhand_dict[k].append(v)

        for k, v in batch_rhand_dict.items():
            if k == 'img_ori':
                continue
            batch_rhand_dict[k] = torch.concat(v, dim=0)
        batch_rhand_dict['bbox_format'] = batch_rhand_list[0]['bbox_format']
        batch_rhand = recursive_to(batch_rhand_dict, "cuda")
        rhand_output = self.forward_step(batch_rhand, decoder_type="hand")
    ...
    def _get_hand_box(self, pose_output, batch):
        ...
        batch_size = batch["affine_trans"].shape[0]
        num_objects = batch["affine_trans"].shape[1]
